...
export const actions = {
    ...
    loadProductDetail: id => {
        return (dispatch, getState) => {
            ...
            const endpint = url.getProductDetail(id);
            return dispatch(fetchProductDetail(endpint, id));
        }
    },
    // 获取店铺信息
    loadShopById: id => {
        return (dispatch, getState) => {
            ...
            const endpint = url.getShopById(id);
            return dispatch(fetchShopById(endpint, id));
        }
    }
}
...
const product = (state = initialState.product, action) => {
    switch (action.type) {
        ...
        case types.FETCH_PRODUCT_DETAIL_SUCCESS:
            return { ...state, isFetching: false, id: action };
